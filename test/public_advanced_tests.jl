...
deviation, the difference between the price drift and mean growth rate, and zero volatility.
"""
function advanced_public_checks()::Vector{NamedTuple}
    ...
    return vcat(standard_public_checks(), [
        (name="Estimate the mean growth rate from price changes", evaluate=() -> isapprox(fitted().mu_g, 0.16/3; atol=1e-12)),
        (name="Scale the sample standard deviation to GBM volatility", evaluate=() -> isapprox(fitted().sigma, sqrt(0.0052/3); atol=1e-12)),
        (name="Add half the squared volatility parameter to obtain the price drift", evaluate=() -> isapprox(fitted().mu, 0.16/3 + 0.0052/6; atol=1e-12)),
        (name="Use the mean growth rate and time in years in the GBM probability", evaluate=() -> isapprox(gbm_probability(parameters, 63, 0.05, 1/252), 0.49002748180476197; atol=1e-12)),
        (name="Return one half when the mean growth rate equals the benchmark", evaluate=() -> isapprox(gbm_probability(parameters, 126, 0.035, 1/252), 0.5; atol=1e-12)),
